...
def vram_safe_generate(gp, gc_config, save_dir, job_id=None):
    ...
    try:
        return generate_music(dit_handler, llm_handler, gp, gc_config, save_dir=save_dir)
    except RuntimeError as e:
        if not _is_vram_error(e):
            raise  # Not a VRAM problem, bubble up as-is
        # Give a clear, actionable error rather than silently retrying
        # (retrying risks hanging on a degraded GPU state)
...
def run_cover(jid, params):
    try:
        ...
        if result.success and result.audios:
            ...
            job_err(jid, getattr(result, 'error', 'Style transfer failed'))
    except Exception as e:
        import traceback; traceback.print_exc()
        job_err(jid, str(e))
...
def run_replace(jid, params):
    try:
        from acestep.inference import GenerationParams, GenerationConfig
        gen_jobs[jid]['message'] = 'Generating replacement stem...'
